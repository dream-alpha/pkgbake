            ...
            if langs:
                for lang in langs:
                    destdir = os.path.join(
                        debdir + installdir, "locale", lang, "LC_MESSAGES")
...
def debbake(argv):
    ...
    if not os.path.isfile(os.path.join(gitdir, "CONTROL/control")):
        print("%s file does not exist, exiting..." %
              os.path.join(gitdir, "CONTROL/control"))
        sys.exit(2)

    deleteDirectory(debdir)
    createDirectory(os.path.join(debdir, "DEBIAN"))
    copyFiles(os.path.join(gitdir, "CONTROL", "*"),
              os.path.join(debdir, "DEBIAN"))
    os.system("echo \"2.0\" > %s" %
              os.path.join(debdir, "DEBIAN/debian-binary"))

    print("processing MAKs...")
    process_maks(gitdir, debdir)

    pkgpath = "None"
    pkg, version, arch = parseControlFile(
        os.path.join(debdir, "DEBIAN/control"))
